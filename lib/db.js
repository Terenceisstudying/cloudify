...
// Use SERVICE_ROLE_KEY primarily for backend operations to bypass RLS
const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY;

// A robust dummy client to prevent crashes if credentials are missing in Vercel
const dummyClient = {
    from: () => ({
        select: () => dummyClient,
        eq: () => dummyClient,
        single: () => Promise.resolve({ data: null, error: { message: 'Supabase credentials not configured in Vercel' } }),
        order: () => dummyClient,
        limit: () => dummyClient,
        insert: () => dummyClient,
        update: () => dummyClient,
        upsert: () => dummyClient,
        delete: () => dummyClient,
        then: (resolve) => resolve({ data: null, error: { message: 'Supabase credentials not configured in Vercel' } })
    }),
    rpc: () => Promise.resolve({ data: null, error: { message: 'Supabase credentials not configured in Vercel' } }),
    auth: {
...
if (process.env.NODE_ENV === 'test') {
    ...
    // (Existing mock logic preserved...)
    const mockDataStore = { settings: [], admins: [], cancer_types: [], questions: [], question_assignments: [], assessments: [], password_reset_tokens: [] };
    class MockQueryBuilder {
        constructor(table) {
            this.table = table; this.filters = []; this._isSingle = false; this._returnsCount = false; this._isHead = false;
            this._limit = null; this._range = null; this._order = []; this._operation = 'select'; this._payload = null;
        }
        select(cols, opts) { if (opts && opts.count) this._returnsCount = true; if (opts && opts.head) this._isHead = true; return this; }
        ...
        async then(resolve) {
            ...
            let tableData = mockDataStore[this.table]; let resultData = [];
            if (this._operation === 'insert') {
                const rows = Array.isArray(this._payload) ? this._payload : [this._payload];
                for (const r of rows) {
                    if (r.id && tableData.some(ex => ex.id === r.id)) return resolve({ data: null, error: { message: 'duplicate key', code: '23505' } });
                    const newRow = { ...r };
                    if (!newRow.id && !['settings', 'question_assignments', 'password_reset_tokens'].includes(this.table)) newRow.id = 'mock-' + Math.random().toString(36).substr(2, 9);
                    if (!newRow.created_at) newRow.created_at = new Date().toISOString();
                    tableData.push(newRow); resultData.push({ ...newRow });
                }
                ...
                for (const r of rows) {
                    ...
                    if (idx !== -1) { tableData[idx] = { ...tableData[idx], ...r, updated_at: new Date().toISOString() }; resultData.push({ ...tableData[idx] }); }
                    else { const newRow = { ...r, created_at: new Date().toISOString() }; tableData.push(newRow); resultData.push({ ...newRow }); }
                }
            } else {
                ...
                tableData.forEach((row, idx) => {
                    ...
                    for (const f of this.filters) {
                        ...
                        else if (f.type === 'ilike') { const regex = new RegExp((f.val || '').replace(/%/g, '.*'), 'i'); if (!regex.test(rowVal)) matches = false; }
                        if (!matches) break;
                    }
                    if (matches) filteredRowsWithIndices.push({ row, idx });
                });
                if (this._operation === 'update') {
                    filteredRowsWithIndices.forEach(({ row }) => { Object.assign(row, this._payload, { updated_at: new Date().toISOString() }); });
                    ...
                    resultData = [];
                } else { resultData = filteredRowsWithIndices.map(({ row }) => ({ ...row })); }
            }
            ...
            } else { resolve({ data: resultData, error: null, count }); }
        }
    }
    clientInstance = {
        from: (table) => new MockQueryBuilder(table),
        rpc: async () => ({ data: [], error: null }),
        auth: { getUser: async () => ({ data: { user: { id: 'test-user' } }, error: null }), signInWithPassword: async () => ({ data: { user: { id: 'test-user' }, session: { access_token: 'mock-token' } }, error: null }) },
        _reset: () => { Object.keys(mockDataStore).forEach(k => { mockDataStore[k] = []; }); }
    };
} else {
    if (supabaseUrl && supabaseKey) {
        clientInstance = createClient(supabaseUrl, supabaseKey);
    } else {
        console.error('Supabase credentials missing! Using dummy client to prevent crash.');
