...
class EmailService {
    ...
    async _send(to, subject, html) {
        return resend.emails.send({
            from: process.env.EMAIL_FROM || 'SCS Risk Assessment <onboarding@resend.dev>',
            to,
            subject,
            html
        });
    ...
    async sendPasswordResetEmail(email, resetToken) {
        ...
        try {
            await this._send(email, 'Password Reset Request - SCS Risk Assessment', html);
    ...
    async sendAssessmentResults(to, data) {
        ...
        const subject = `Your ${isGeneric ? 'General' : assessmentType?.charAt(0).toUpperCase() + assessmentType?.slice(1) || ''} Cancer Risk Assessment Results`;
        return this._send(to, subject, htmlContent);
    }
